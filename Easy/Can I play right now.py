...
# I define a prime-time as a time period till which Vasya have a permission to play computer games. I specify start hour and end hour as pair of integers.
...
def can_i_play(now_hour, start_hour, end_hour):
    if start_hour > end_hour:
        if now_hour >= end_hour:
            if start_hour <= now_hour < end_hour+24:
                return True
        else:
            if start_hour-24 <= now_hour < end_hour:
                return True
